/**
 * Smart Timetable CMS - SaaS Version
 * Version: 10.0 (BYOD Multi-Tenant Cloud Architecture + Intelligent Auto-Swap)
 */

// ==========================================
// 🌟 SAAS CLOUD ARCHITECTURE (LOCAL STORAGE)
// ==========================================
let SCRIPT_URL = localStorage.getItem('school_backend_url');

window.connectNewSchool = function() {
    let url = document.getElementById('setupScriptUrl').value.trim();
    if(!url || !url.startsWith('https://script.google.com/')) {
        return alert('Please enter a valid Google Apps Script Web App URL.');
    }
    localStorage.setItem('school_backend_url', url);
    alert("Database Connected Successfully! Welcome to Smart Timetable Cloud.");
    location.reload(); // Reload to show login screen
};

window.disconnectSchool = function() {
    if(confirm("⚠️ Disconnect Database?\n\nAre you sure you want to disconnect this device from your school's database? You will need to paste your URL again to reconnect.")) {
        localStorage.removeItem('school_backend_url');
        location.reload();
    }
};

// ==========================================
// 🌟 UI DROPDOWN LISTENERS & APP INITIALIZATION
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
    
    // SaaS Logic: Show Setup or Login based on Database connection
    if (!SCRIPT_URL) {
...
window.attemptLogin = async function() {
    if(!SCRIPT_URL) return alert("Please setup your database connection first.");
...
window.editAllotmentRow = function(index) {
    ...
    document.getElementById('newTName').focus(); 
};
...
window.openOptimizerModal = function() {
    ...
    window.uiAllotments.forEach(req => {
        let blocks = [{ act: req.act1, cls: req.cls1, per: parseInt(req.per1)||0 }, { act: req.act2, cls: req.cls2, per: parseInt(req.per2)||0 }, { act: req.act3, cls: req.cls3, per: parseInt(req.per3)||0 }];
        blocks.forEach(b => {
            if (b.per > 0 && b.cls) {
                getIndividualClasses(b.cls).forEach(c => {
                    if (!classData[c]) classData[c] = { total: 0, subjects: {} };
                    classData[c].total += b.per; classData[c].subjects[b.act] = (classData[c].subjects[b.act] || 0) + b.per;
                });
            }
        });
    });
    ...
    document.getElementById('optimizerModal').classList.remove('hidden'); if (window.lucide) window.lucide.createIcons();
};
window.closeOptimizerModal = function() { document.getElementById('optimizerModal').classList.add('hidden'); };

// ==========================================
// 🌟 GRID RENDERING & MANUAL EDITING (AUTO-SWAP)
// ==========================================
...
window.saveCellEdit = function() {
    ...
    let oldTeacher = null; let oldSub = null;
    ...
    // 🌟 SILENT CLASH RESOLVER (AUTO-SWAP EXCEPTION FOR PET/CLUB)
...
window.saveGridToCloud = async function() {
    ...
    try { await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: "saveGrid", data: generatedWeeklyTimetable }) }); alert("Timetable Grid successfully saved to Cloud!"); updateStatus("System Ready"); } catch (e) { alert("Error saving Grid."); }
};
...
window.exportPDF = function(mode) {
    ...
    if (mode === 'exam') {
        ...
        const doc = new jsPDF('l', 'mm', 'a4'); doc.setFontSize(14); doc.text(`Exam Invigilation Schedule`, 14, 15); doc.setFontSize(11); doc.text(`Date: ${selectedDate} | Session: ${document.getElementById('examSession').value}`, 14, 25); doc.save(`Exam_Schedule.pdf`);
        return;
    } 
    if (mode === 'sub') {
        ...
        const doc = new jsPDF('l', 'mm', 'a4'); doc.setFontSize(14); doc.text(`Substitution Duty - ${selectedDate} (${day})`, 14, 15); doc.save(`Sub_Schedule.pdf`);
        return;
    }
    ...
    if (viewType === 'all' || !filterVal) {
        ...
        allTeachers.forEach((teacher) => {
            ...
            doc.setDrawColor(180, 180, 180); doc.setLineWidth(0.3); doc.rect(x, y, cW, cH); doc.setFontSize(9); doc.setTextColor(0); doc.setFont("helvetica", "bold"); doc.text(`Teacher: ${teacher.length > 20 ? teacher.substring(0, 18) + "..." : teacher}`, x + 2, y + 5);
        ...
        doc.save(`All_Teacher_Cards.pdf`);
    } else {
        const doc = new jsPDF('l', 'mm', 'a4'); doc.setFontSize(16); doc.text(`Timetable - ${filterVal}`, 14, 18); doc.autoTable({ html: '#scheduleTable', startY: 25, theme: 'grid', styles: { fontSize: 10, cellPadding: 4, halign: 'center', valign: 'middle' }, headStyles: { fillColor: [41, 128, 185], textColor: 255 }}); doc.save(`Schedule_${filterVal.replace(' ', '_')}.pdf`);
    }
};